import { type AuthService, PKCEService, type StorageAdapter } from "@auth/index";
...
import type { GetChapterRequest, GetSeveralChapterRequest } from "@schemas/services/chapter";
import type {
...
class BetterMusicClient {
  private readonly runtime: ManagedRuntime.ManagedRuntime<AppServices, ConfigError | PlatformError>;

  constructor(authLayer: Layer.Layer<AuthService, ConfigError | PlatformError>) {
  ...
  private async run<A, E>(effect: Effect.Effect<A, E, AppServices>): Promise<A> {
  ...
  get artist() {
    return {
      ...
      getAlbums: (request: GetArtistAlbumRequest, options?: AlbumRetrievalOptions) =>
        this.run(
          Effect.gen(function* () {
            const s = yield* ArtistService;
      ...
      getTopTracks: (request: GetArtistTopTracksRequest, options?: MarketOnlyOptions) =>
        this.run(
          Effect.gen(function* () {
            const s = yield* ArtistService;
  ...
  get audiobook() {
    return {
      ...
      getMany: (request: GetSeveralAudiobookRequest, options?: MarketOnlyOptions) =>
        this.run(
          Effect.gen(function* () {
            const s = yield* AudiobookService;
            return yield* s.getMany(request, options);
      ...
      getChapters: (request: GetAudiobookChapterRequest, options?: PaginatedMarketOptions) =>
        this.run(
          Effect.gen(function* () {
            const s = yield* AudiobookService;
  ...
  get show() {
    return {
      ...
      getEpisodes: (request: GetShowEpisodeRequest, options?: PaginatedMarketOptions) =>
        this.run(
          Effect.gen(function* () {
            const s = yield* ShowService;
