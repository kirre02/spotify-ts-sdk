...
export function makeRequest<T, I, R>({
  ...
  return Effect.gen(function* () {
    ...
    const text = yield* Effect.tryPromise({
      try: () => response.text(),
      catch: (cause) => new JsonParseError({ message: "Failed to read response body", cause }),
    });
    ...
    if (!response.ok) {
      ...
      switch (status) {
        ...
        case 429: {
          ...
          const retryHeader = retryAfterHeader !== null ? Number(retryAfterHeader) || 15 : 15;
    ...
    Effect.tapError((error) => {
      if (error._tag === "RateLimitError") return Effect.sleep(error.retryAfter);
      return Effect.void;
    }),
    Effect.retry({
      schedule: Schedule.union(
        Schedule.recurs(3).pipe(
          Schedule.whileInput((error: ApiError) => error._tag === "RateLimitError"),
        ),
        Schedule.exponential(Duration.seconds(1), 2).pipe(
          Schedule.intersect(Schedule.recurs(3)),
          Schedule.whileInput((error: ApiError) => error._tag === "NetworkError"),
        ),
      ),
    }),
  );
