...
export class AlbumService extends Context.Tag("AlbumService")<
  AlbumService,
  {
    ...
    readonly save: (request: SaveAlbumRequest) => Effect.Effect<void, ApiError, AuthService>;
    readonly remove: (request: RemoveAlbumRequest) => Effect.Effect<void, ApiError, AuthService>;
    readonly checkSaved: (
  ...
  Effect.succeed(
    AlbumService.of({
      get: (request: GetAlbumRequest, options?: MarketOnlyOptions) => {
        ...
        if (options?.market != null) guardMarket(options.market, "[AlbumService/Get]");
      ...
      getMany: (request: GetSeveralAlbumRequest, options?: MarketOnlyOptions) => {
        ...
        if (options?.market != null) guardMarket(options.market, "[AlbumService/GetMany]");
      ...
      getTracks: (request: GetAlbumTracksRequest, options?: PaginatedMarketOptions) => {
        const { id } = request;

        guardId(id, "[AlbumService/GetTracks] Album id");
        if (options?.limit != null) guardLimit(options.limit, 50, "[AlbumService/GetTracks]");
        if (options?.market != null) guardMarket(options.market, "[AlbumService/GetTracks]");
        if (options?.offset != null) guardOffset(options.offset, "[AlbumService/GetTracks]");
      ...
      getSaved: (options?: PaginatedMarketOptions) => {
        if (options?.limit != null) guardLimit(options.limit, 50, "[AlbumService/GetSaved]");
        if (options?.market != null) guardMarket(options.market, "[AlbumService/GetSaved]");
        if (options?.offset != null) guardOffset(options.offset, "[AlbumService/GetSaved]");
      ...
      getNewReleases: (options?: PaginationOptions) => {
        if (options?.limit != null) guardLimit(options.limit, 50, "[AlbumService/GetNewReleases]");
        if (options?.offset != null) guardOffset(options.offset, "[AlbumService/GetNewReleases]");
