...
import { guardArrays, guardLimit, guardMarket, guardOffset, guardString } from "@guards";
  ...
  Effect.succeed(
    SearchService.of({
      query: (request: SearchRequest, options?: MarketExternalOptions) => {
        ...
        if (options?.limit != null) guardLimit(options.limit, 50, "[SearchService/Query]");
        if (options?.offset != null) guardOffset(options.offset, "[SearchService/Query]");
        ...
        if (options?.market != null) guardMarket(options.market, "[SearchService/Query]");
        if (options?.include_external != null && options.include_external !== "audio")
          throw new IllegalArgumentException(
        ...
        const encodedTypes = types.map((type) => type.trim()).join(encodeURIComponent(","));
