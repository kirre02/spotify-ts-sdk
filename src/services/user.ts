...
import { guardId, guardIds, guardLimit, guardOffset, guardString } from "@guards";
import type { AfterBasedPaginationOptions, TimeRangePaginationOptions } from "@schemas/options";
import {
...
export class UserService extends Context.Tag("UserService")<
  UserService,
  {
    readonly getCurrentUser: () => Effect.Effect<GetCurrentUserResponse, ApiError, AuthService>;
  ...
  Effect.succeed(
    UserService.of({
      ...
      getTopArtists: (options?: TimeRangePaginationOptions) => {
        if (options?.limit != null) guardLimit(options.limit, 50, "[UserService/GetTopArtists]");
        if (options?.offset != null) guardOffset(options.offset, "[UserService/GetTopArtists]");
      ...
      getTopTracks: (options?: TimeRangePaginationOptions) => {
        if (options?.limit != null) guardLimit(options.limit, 50, "[UserService/GetTopTracks]");
        if (options?.offset != null) guardOffset(options.offset, "[UserService/GetTopTracks]");
