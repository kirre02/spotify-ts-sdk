...
import { guardArrays, guardId, guardIds, guardLimit, guardMarket, guardOffset } from "@guards";
import type { AlbumRetrievalOptions, MarketOnlyOptions } from "@schemas/options";
import {
  ...
  Effect.succeed(
    ArtistService.of({
      ...
      getAlbums: (request: GetArtistAlbumRequest, options?: AlbumRetrievalOptions) => {
        ...
        if (options?.limit != null) guardLimit(options.limit, 10, "[ArtistService/GetAlbums]");
        if (options?.market != null) guardMarket(options.market, "[ArtistService/GetAlbums]");
        if (options?.offset != null) guardOffset(options.offset, "[ArtistService/GetAlbums]");
      ...
      getTopTracks: (request: GetArtistTopTracksRequest, options?: MarketOnlyOptions) => {
        ...
        if (options?.market != null) guardMarket(options.market, "[ArtistService/GetTopTracks]");
