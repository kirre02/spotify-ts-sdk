...
function refreshToken({ clientId, refreshToken }: { clientId: string; refreshToken: string }) {
  return Effect.gen(function* () {
      ...
      Effect.mapError(
        (cause) =>
          new SchemaDecodeError({
            message: "Failed to decode Spotify PKCE token response during refresh",
            cause,
          }),
      ),
    );
...
export function makePKCEAuth(adapter: StorageAdapter, scopes: string[]) {
  const authLayer = Layer.effect(
    AuthService,
    Effect.gen(function* () {
      const clientId = Redacted.value(yield* Config.redacted("SPOTIFY_CLIENT_ID"));
  ...
  const pkceLayer = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const redirectUri = Redacted.value(yield* Config.redacted("SPOTIFY_REDIRECT_URI"));
      ...
      const exchangeCodeForTokens = (params: { code: string; state: string }) =>
        Effect.gen(function* () {
          ...
          if (!stored) {
            return yield* Effect.fail(
              new TokenNotFoundError({
                message: "Missing PKCE verifier. Call getAuthorizationUrl() first.",
              }),
            );
          }
          ...
          const token = yield* Schema.decodeUnknown(PKCETokenExtensionSchema)(json).pipe(
            Effect.mapError(
              (cause) =>
                new SchemaDecodeError({
