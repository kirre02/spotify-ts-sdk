...
import { JsonParseError, NetworkError, SchemaDecodeError, UnknownApiError } from "@errors";
      ...
      Effect.mapError(
        (cause) =>
          new SchemaDecodeError({
            message: "Failed to decode Spotify Client Credential token response",
            cause,
          }),
      ),
    );
      ...
      return AuthService.of({
        getToken: Effect.gen(function* () {
          ...
          if (!maybeToken || maybeToken.expires_at < Date.now()) {
            const token = yield* fetchToken(Redacted.value(clientId), Redacted.value(clientSecret));
