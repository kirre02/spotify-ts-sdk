...
export type GetTrackResponse = Schema.Schema.Type<typeof GetTrackResponseSchema>;
...
export type GetSeveralTrackResponse = Schema.Schema.Type<typeof GetSeveralTrackResponseSchema>;

export const GetSavedTrackResponseSchema = PageSchema(SavedTrackSchema);
export type GetSavedTrackResponse = Schema.Schema.Type<typeof GetSavedTrackResponseSchema>;

export type SaveTrackRequest = {
  /**
   * A list of the Spotify IDs for the tracks. Maximum: 50 IDs. If `timestamped_ids` is present any
   * IDs listed in this field will be ignored
   *
   * @see {@link https://developer.spotify.com/documentation/web-api/concepts/spotify-uris-ids|Spotify IDs}
   */
  ids?: string[];
  /**
   * A list of objects containing track IDs with their corresponding timestamps. Each object must
   * include a track ID and an added_at timestamp. This allows you to specify when tracks were added
   * to maintain a specific chronological order. Maximum: 50 IDs. If this field is used any IDs
   * present in the `ids` field will be ignored.
   */
  timestamped_ids?: {
    /**
     * The Spotify ID for the track.
    ...
    id: string;
    /**
     * The timestamp when the track was added to the library. Use ISO 8601 format with UTC timezone
     * (e.g., 2023-01-15T14:30:00Z). You can specify past timestamps to insert tracks at specific
     * positions in the library's chronological order.
     */
    added_at: string;
  }[];
};
